...
export function parseTable(path) {
  ...
  for (let i = 0; i < parsedTable.length; i++)
    parsedTable[i][3][1] = pathAnalyzer.pathHasArrayForEach(parsedTable[i][1])
...
function _parsePathNamesAndSort(parsedTable) {
  ...
  for (let i = 0; i < parsedTable.length; i++) {
    if (!parsedTable[i][3][0] && !parsedTable[i][3][1])
      parsedTable[i][0] = pathStringifier.stringifyPath(
...
function _zeroedMatchingPath(parsedField, parsedGroupBy) {
  ...
  for (
    ...
    while (currentParsedGroupBy[j] == parser.RULE) j++
    if (
      parsedField[i] === parser.ARRAY_ALL &&
      (currentParsedGroupBy[j] === parser.ARRAY_ALL ||
        currentParsedGroupBy[j] === parser.ARRAY_FOREACH)
    ) {
      parsedField.splice(i, 2, parser.ARRAY_INDEX, 0, ...parsedField[i + 1])
      i++
