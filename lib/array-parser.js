...
export function parseArray(path, startIndex) {
  ...
  for (; i < len; i++) {
    ...
    } else if (depth === 1 && objectDepth === 0) {
      if (items.length === 0) {
        if (token === '|' && path[i + 1] === '>') {
          pipeStart = ++i + 1
        } else if (token === '=' && path[i + 1] === '>') {
          isFullPipe = false
          pipeStart = ++i + 1
        }
      }
  ...
  if (items.length === 0) return [[ARRAY_ALL, []], i]
    ...
    if (pipe !== '') {
      if (currentPrePipe === '')
        return [[ARRAY_ALL, [], [isFullPipe, parseFunction(pipe)]], i]
      return [
...
function _getSingleArrayAccessWithPipe(value, valuePrePipe, isFullPipe, pipe) {
  ...
  singleArrayAccess.push([isFullPipe, pipe])
