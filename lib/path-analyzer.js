  ...
  for (const token of parsedPath) {
    switch (token) {
      ...
      case parser.ARRAY_REPLACE:
        ...
        break
      default:
        if (typeof token === 'object' && token.multi) depth++
    }
  }
...
export function pathHasArrayAll(path) {
  ...
  for (const token of parsedPath)
    if (token === parser.ARRAY_ALL || token === parser.ARRAY_REPLACE)
      return true

  return false
}

export function pathHasArrayReplace(path) {
  if (path === undefined || path === '') return false

  const parsedPath = Array.isArray(path) ? path : parser.parse(path)

  for (const token of parsedPath)
    if (token === parser.ARRAY_REPLACE) return true
