import { EMPTY, NUMBER, OBJECT } from './_types.js'
  ...
  while (true) {
    if (keyIndex >= keysLength) {
      if (keyIndex === 0) {
        ...
        else flat[path[depth] + MULTI] = EMPTY
      }
...
function _getKey(key, pathLength) {
  ...
  if (pathLength > 1 && tempKey[0] !== ESCAPED_OBJECT_PROPERTY_START)
    return OBJECT_PROPERTY + tempKey
...
function _tryValueArray(topList) {
  ...
  return [ARRAY_ALL + path.join(EMPTY), values]
}
