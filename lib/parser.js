...
function isDefault(ctx) {
  ctx.isDefault && !",}'".includes(ctx.path[ctx.i])
}

function _handleDefault(ctx) {
  ctx.current += ctx.path[ctx.i]
}
...
function _handleClosingArrayAccess(ctx) {
  if (isInt.test(ctx.current)) {
    const integer = parseInt(ctx.current, 10)
...
function _handleOpeningObject(ctx) {
  ...
  if (ctx.path.slice(ctx.i, ctx.i + 3) === '{:}') {
    ctx.parsedPath.push(OBJECT_UN_FLAT)
    ctx.current = ''
    ctx.isTabletize = false
    ctx.i += 2
    return
  ...
  ctx.i++
  do {
    ctx.multiBack = parse(ctx.path.slice(ctx.i), true, ctx.currentDeep + 1)
    ...
    if (ctx.multiBack[4] > ctx.deepestDeep) ctx.deepestDeep = ctx.multiBack[4]
...
function _isColonMulti(ctx) {
  return !ctx.multi || ctx.multiPath
}
