...
import { parseArray, DEEP_OBJECT_PROPERTY } from './array-parser.js'
...
export function _safe(value, trim = true) {
  ...
  return trim ? trimValue : value
}

function _safePush(parsedPath, value, trim = true) {
  const safeValue = _safe(value, trim)
  if (safeValue === '') return

  if (safeValue === DEEP_OBJECT_PROPERTY_UNSCAPED)
    parsedPath.push(DEEP_OBJECT_PROPERTY, true)
