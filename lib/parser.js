...
// TODO: export const ARRAY_REPLACE_UNIQUE = '%[*#]%'
    ...
    switch (path[i]) {
      ...
      case ']':
        if (isInt.test(current)) {
          ...
          parsedPath.push(ARRAY_ADD)
          currentDeep++
        } else if (current === '*') {
          parsedPath.push(ARRAY_REPLACE)
          currentDeep++
        } else if (current === ':') {
          parsedPath.push(ARRAY_UN_FLAT)
          currentDeep++
        } else if (current === '#') {
          parsedPath.push(ARRAY_UNIQUE)
          currentDeep++
        } else if (current === ':#' || current === '#:') {
          parsedPath.push(ARRAY_UN_FLAT_UNIQUE)
          currentDeep++
      ...
      case '{':
        ...
        if (path.slice(i, i + 3) === '{:}') {
          parsedPath.push(OBJECT_UN_FLAT)
          current = ''
          isTabletize = false
          i += 2
          continue
