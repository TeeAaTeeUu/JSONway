...
import { OBJECT_START, parseObject } from './object-parser.js'
import { TABLE } from './table-parser.js'
import { getInteger } from './value-parser.js'
import { BOOLEAN, EMPTY, NUMBER, STRING } from './_types.js'

export const OBJECT_PROPERTY = '.'
export const PIPE = '|'
export const ESCAPED_OBJECT_PROPERTY_START = '['
export const ESCAPED_OBJECT_PROPERTY_END = ']'
    ...
    switch (path[i]) {
      ...
      case EQUAL:
        ...
        switch (path[i]) {
          ...
          case EQUAL:
            if (path[i + 1] === PIPE) i++
            else if (path[i + 1] === LARGER) i++

            previousParsedPath = parsedPath
            parsedPath.push(PIPE, [true, []])
...
function _potentialImpliedArrayAll(parsedPath, previousParsedPath) {
  return (
    parsedPath.at(-2) === OBJECT_PROPERTY &&
    (previousParsedPath.at(-2) === ARRAY_LIST || parsedPath.at(-4) === TABLE)
  )
}
