  ...
  for (let i = 0; i < subResults.length; i++) {
    ...
    for (let j = 0; j < results.length; j++) {
      ...
      if (!parsedPaths[i][2]) {
        ...
        else subResult = subResults[i][currentIndex]

        if (subResult === undefined) true
  ...
  for (let i = 0; i < subResults.length; i++) {
    ...
    if (subResults[i][1] === undefined)
      ...
      for (let j = 0; j < subResults[i][1].length; j++) {
        ...
        if (!temp[key]) temp[key] = [[subResults[i][0][j], subResults[i][1][j]]]
        else temp[key].push([subResults[i][0][j], subResults[i][1][j]])
      }
...
export function parsePathNamesAndSort(rawParsePaths) {
  const paths = rawParsePaths[1].map((_, i) => [
    ...
    pathAnalyzer.pathHasArrayReplace(rawParsePaths[1][i]),
  ])
...
function _zeroedPath(parsedPath) {
  return pathStringifier.stringifyPath(
    parsedPath.map(token =>
      token !== parser.ARRAY_ALL && token !== parser.ARRAY_REPLACE ? token : 0,
    ),
  )
}
...
function _zeroedMatchingPath(parsedField, parsedGroupBy) {
  for (
    ...
    while (parsedGroupBy[j].rule) j++
    if (
      parsedField[i] === parser.ARRAY_ALL &&
      (parsedGroupBy[j] === parser.ARRAY_ALL ||
        parsedGroupBy[j] === parser.ARRAY_REPLACE)
    ) {
      parsedField[i] = 0
      continue
    }
    if (parsedField[i] === parsedGroupBy[j]) continue
    if (parsedField[i] === 0 && parsedGroupBy[j] === parser.ARRAY_ALL) continue
