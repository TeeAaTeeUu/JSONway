import { EMPTY } from './_types.js'
import { getArrayIndexes } from './getter.js'
import { pathDepth } from './path-analyzer.js'
  ...
  for (let i = 0; i < subResults.length; i++) {
    ...
    for (let j = 0; j < results.length; j++) {
      if (!parsedPaths[i][3][1]) {
        if (subKeys.length === 1 && subKeys[0] === EMPTY) {
          if (
...
function _getSubResults(data, parsedPaths) {
  const subResults = parsedPaths.map(parsedPath =>
    getArrayIndexes(data, parsedPath[1]),
  )
  ...
  for (let i = 0; i < subResults.length; i++) {
    ...
    if (subResults[i][1] === undefined)
      temp[EMPTY] = [[subResults[i][0], subResults[i][1]]]
    else
...
function _getFlatDepthNeeded(parsedPaths) {
  return parsedPaths
    .map(parsedPath => pathDepth(parsedPath[1]) - 2)
