...
function _deepValues(parsedPath, i, originalCurrent) {
  ...
  const pipe = Array.isArray(parsedPath[i + 1]) ? parsedPath[i + 1] : false
  if (pipe) return _getPiped(result, pipe)
...
function _getPiped(result, pipe) {
  if (pipe[0] && pipe[1][0] === OBJECT_PROPERTY)
    return processFunction(result, pipe[1])
  if (pipe[0]) return get(result, pipe[1])

  // TODO: support each-pipe
