  ...
  for (let i = 0; i <= lastPathIndex; i++) {
    switch (parsedPath[i]) {
      ...
      case ARRAY_INDEXES:
        ...
        for (const j of parsedPath[i][0]) {
          set(previous, [ARRAY_INDEX, j, ...parsedPath[i].slice(1)], value)
        }
        continue
      case ARRAY_FOREACH:
        if (parsedPath[++i].length === 0) {
...
function getIgnoreAddingTemp(parsedPath) {
  for (let i = 0, len = parsedPath.length; i < len; i++)
    if (parsedPath[i] === ARRAY_FOREACH) return true
...
function getNextSetOperation(parsedPath, i) {
  ...
  while (currentParsedPath[j] === RULE || Array.isArray(currentParsedPath[j])) {
    if (currentParsedPath[j] === RULE) {
      j += 2
