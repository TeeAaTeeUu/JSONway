...
describe('setter', () => {
  ...
  it('a[]', function () {
    ...
    out = { a: [5] }
    assert.deepEqual(JSONway.set({}, this.test.title, 5), out)

    out = { a: ['zo', 5] }
    assert.deepEqual(JSONway.set({}, this.test.title, ['zo', 5]), out)
  })
  ...
  it('a.2', function () {
    const out = { a: [undefined, undefined, 'z'] }
    assert.deepEqual(JSONway.set({}, this.test.title, 'z'), out)
  })
  ...
  it('a(c=5).b', function () {
    ...
    const input = 'a(c: 5).b'
    out = { a: { b: 'x', c: 5 } }
    assert.deepEqual(JSONway.set({}, input, 'x'), out)
  })
  ...
  it(`a(c='yy'&&d=5).b`, function () {
    ...
    const input = `a(c: 'yy', d: 5).b`
    out = { a: { b: 'x', c: 'yy', d: 5 } }
    assert.deepEqual(JSONway.set({}, input, 'x'), out)
  })

  it('a(c=10&&d>5).b', function () {
    const out = { a: { b: 'x', c: 10 } }
    assert.deepEqual(JSONway.set({}, this.test.title, 'x'), out)
  })

  it('a(c=10&&).b', function () {
    const out = { a: { b: 'x', c: 10 } }
    assert.deepEqual(JSONway.set({}, this.test.title, 'x'), out)
  })
